...
Current packaging baseline: `1.0.0-mvp` from [VERSION](d:/Code/Git_Project/Realtime_Transcription/VERSION).
...
For release-style local startup and Windows packaging entry, use:

```powershell
python launcher.py
```

`launcher.py` starts the current local app server without reload mode and opens the browser when the local URL is ready.
